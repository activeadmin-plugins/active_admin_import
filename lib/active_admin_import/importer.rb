...
module ActiveAdminImport
  class Importer
    attr_reader :resource, :options, :result, :model, :headers_array
    ...
    def process_file
      ...
      File.open(file.path) do |f|
        ...
        @headers_array = CSV.parse(f.readline, @csv_options).first
        prepare_headers { @headers_array }
        f.each_line do |line|
    ...
    def prepare_headers
      @headers = headers.present? ? headers.map(&:to_s) : yield
      @headers = construct_hash(@headers)
      @headers.merge!(options[:headers_rewrites]
              .symbolize_keys
              .slice(*@headers.symbolize_keys.keys))
      @headers
    end

    def construct_hash
      Hash[
        headers.zip(headers.map { |el| el.underscore.gsub(/\s+/, '_') })
